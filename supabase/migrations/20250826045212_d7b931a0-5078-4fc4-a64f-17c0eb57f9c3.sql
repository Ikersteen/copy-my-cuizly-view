-- Update email templates with rich Cuizly design using PostgreSQL functions

-- Create a function to generate the confirmation email template
CREATE OR REPLACE FUNCTION generate_confirmation_email_template()
RETURNS jsonb AS $$
BEGIN
...
-- Create a function to generate the password reset email template
CREATE OR REPLACE FUNCTION generate_password_reset_email_template()
RETURNS jsonb AS $$
BEGIN
  RETURN jsonb_build_object(
...
-- Create a function to generate the magic link email template
CREATE OR REPLACE FUNCTION generate_magic_link_email_template()
RETURNS jsonb AS $$
BEGIN
  RETURN jsonb_build_object(
...
-- Update auth config using the functions
UPDATE auth.config 
SET 
  external_email_enabled = true,
  external_email_confirm_signup_template = generate_confirmation_email_template()::text,
  external_email_reset_password_template = generate_password_reset_email_template()::text,
  external_email_magic_link_template = generate_magic_link_email_template()::text
WHERE id = 1;