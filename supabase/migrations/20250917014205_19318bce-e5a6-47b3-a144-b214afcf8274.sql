...
-- Utiliser un utilisateur existant ou créer des restaurants avec un owner_id spécifique
DO $$
DECLARE
    sample_owner_id uuid;
BEGIN
    -- Récupérer un utilisateur existant ou utiliser un ID par défaut
    SELECT user_id INTO sample_owner_id FROM profiles LIMIT 1;
    
    -- Si pas d'utilisateur trouvé, créer avec un UUID générique (sera modifié plus tard par les vrais propriétaires)
